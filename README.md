...
## Semantic Execution Gateway (in progress, `feature/semantic-gateway`)
...
compiler turns that into the query, and the result goes through row-level
security, the same guardrails above, and a semantic cache before hitting
BigQuery. It also adds a stacking pattern — combining retrieval over support
notes with the compiled query path so a question like "why are our
...
This is mid-build behind a feature flag (`main` and everything above stays
live and unaffected); numbers and a full write-up land when it's finished.
